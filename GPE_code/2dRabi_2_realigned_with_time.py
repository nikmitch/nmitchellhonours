...
t=0.0
times=np.linspace(0,T,nStep+1)
first_site_probs=np.zeros((1,nStep+1))
tic=time.time()
...
#  print(renormfactor)
  ...
  if ind%100==0:
      print("writing sparse method at t=%.3f, " %t + "current norm:%.6f" %renormfactor)
  t+=dt

print("Time evolution required:         ", time.time() - tic, " seconds.")  
first_site_probs=np.asarray(first_site_probs)[0]
